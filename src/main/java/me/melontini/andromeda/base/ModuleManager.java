...
public final class ModuleManager implements ModuleApiProvider {
  ...
  ModuleManager(List<Module.Zygote> zygotes) {
    ...
    this.discoveredModules = Utilities.supply(() -> {
      var m = zygotes.stream()
          .collect(Collectors.toMap(
              Module.Zygote::type, Promise::new, (t, t2) -> t, LinkedHashMap::new));
    ...
    this.discoveredModuleNames = Utilities.supply(() -> {
      var m = zygotes.stream()
          .collect(Collectors.toMap(
              ...
              o -> this.discoveredModules.get(o.type()),
              (t, t2) -> t,
              HashMap::new));
      return Collections.unmodifiableMap(m);
    });

    List<? extends Module> sorted = zygotes.stream()
        .map(Module.Zygote::supplier)
        .map(s -> {
    ...
    sorted.forEach(m -> configs.put(m, CompletableFuture.supplyAsync(() -> {
      var path = FabricLoader.getInstance()
          .getConfigDir()
          .resolve("andromeda/" + m.meta().id() + ".json");
      ...
      try (var reader = Files.newBufferedReader(path)) {
        ...
        LOGGER.error(
            "Failed to load {}! Resetting to default!",
            FabricLoader.getInstance().getGameDir().relativize(path),
            e);
        return BootstrapConfig.create(m.getClass().isAnnotationPresent(EnabledByDefault.class));
      }
    })));
    Map<Module, BootstrapConfig> bootstrapConfigs =
        new IdentityHashMap<>(Maps.transformValues(configs, CompletableFuture::join));
    this.configGetter = bootstrapConfigs::get;

    if (Debug.Keys.ENABLE_ALL_MODULES.isPresent())
      bootstrapConfigs.values().forEach(c -> c.enabled = true);
  ...
  static void validateZygote(@NonNull Module.Zygote module) {
    MakeSure.notEmpty(
        ...
        "Module category can't be null or empty! Module: " + module.getClass());
    MakeSure.isTrue(
        !module.meta().category().contains("/"),
  ...
  public void cleanConfigs(Path root, Collection<? extends Module> modules) {
    if (Files.exists(root)) {
      Set<Path> paths = collectPaths(
          Objects.requireNonNull(root.getParent(), () -> "Root config folder? %s".formatted(root)),
          modules);
      Exceptions.runAsResult(() -> Files.walkFileTree(root, new SimpleFileVisitor<>() {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
              ...
              if (file.toString().endsWith(".json")
                ...
                LOGGER.info(EarlyLanguage.translate(
                    "andromeda.module_manager.removed_unlinked_config",
                    FabricLoader.getInstance().getGameDir().relativize(file)));
              }
          ...
          .ifPresent(t -> LOGGER.error("Failed to clean up configs!", t));
    }
  }

  private Set<Path> collectPaths(
      @NonNull Path root, @NonNull Collection<? extends Module> modules) {
    ...
    modules.forEach(
        module -> paths.add(root.resolve("andromeda/" + module.meta().id() + ".json")));
  ...
  public void saveBootstrap(Module module) {
    var path = FabricLoader.getInstance()
        .getConfigDir()
        .resolve("andromeda/" + module.meta().id() + ".json");
    ...
    try {
      ...
      LOGGER.error(
          "Failed to save {}!", FabricLoader.getInstance().getGameDir().relativize(path), e);
    }
  }
  ...
  public static <T extends Module> T quick(Class<T> cls) {
    return get()
        ...
        .orElseThrow(() -> new IllegalStateException(
            "Module %s requested quickly, but is not loaded.".formatted(cls)));
  }
  ...
  void print() {
    ...
    if (!categories.isEmpty()) {
      LOGGER.info(
          EarlyLanguage.translate(
                  "andromeda.module_manager.loading_modules", loaded().size()) + " {}",
          builder);
    } else {
