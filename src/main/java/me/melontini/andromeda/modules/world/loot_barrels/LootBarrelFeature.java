...
public class LootBarrelFeature extends Feature<LootBarrelFeature.LootBarrelConfiguration> {
  ...
  public boolean generate(FeatureContext<LootBarrelConfiguration> context) {
    ...
    if (!world.getBlockState(context.getOrigin()).isAir()
        || !world.getBlockState(context.getOrigin().down()).isOpaque()) return false;

    world.setBlockState(
        context.getOrigin(),
        Blocks.BARREL
            .getDefaultState()
            .with(BarrelBlock.FACING, Util.getRandom(HORIZONTAL, context.getRandom())),
        Block.NOTIFY_LISTENERS);
    LootableContainerBlockEntity.setLootTable(
        world, context.getRandom(), context.getOrigin(), context.getConfig().loot());

    if (world.getBlockState(context.getOrigin().up()).isAir()
        && context.getConfig().decorations().stream().findAny().isPresent()) {
      world.setBlockState(
          context.getOrigin().up(),
  ...
  public record LootBarrelConfiguration(Identifier loot, WeightedList<BlockState> decorations)
  ...
  public static void init() {
    ...
    BiomeModifications.addFeature(
        context -> context.canGenerateIn(DimensionOptions.OVERWORLD),
