...
public class LootContextUtil {
  ...
  public static Supplier<LootContext> command(World world, Vec3d origin, @Nullable Entity entity) {
    return () -> build(setBuilder((ServerWorld) world)
        .add(ORIGIN, origin)
  ...
  public static Supplier<LootContext> fishing(
      World world, Vec3d origin, @Nullable ItemStack tool, @Nullable Entity entity) {
    return () -> build(setBuilder((ServerWorld) world)
        .add(ORIGIN, origin)
  ...
  public static Supplier<LootContext> entity(
      World world,
      Vec3d origin,
      Entity entity,
      ...
      @Nullable PlayerEntity lastDamagePlayer) {
    return () -> build(setBuilder((ServerWorld) world)
        .add(ORIGIN, origin)
        .add(THIS_ENTITY, entity)
        .add(
            DAMAGE_SOURCE,
            Objects.requireNonNullElseGet(source, () -> world.getDamageSources().generic()))
        .addOptional(KILLER_ENTITY, killer)
        .addOptional(DIRECT_KILLER_ENTITY, directKiller)
  ...
  public static Supplier<LootContext> entity(
      World world, Vec3d origin, Entity entity, @Nullable DamageSource source) {
    return () -> build(setBuilder((ServerWorld) world)
        .add(ORIGIN, origin)
        .add(THIS_ENTITY, entity)
        .add(
  ...
  public static Supplier<LootContext> block(
      World world,
      Vec3d origin,
      BlockState state,
    ...
    return () -> build(setBuilder((ServerWorld) world)
        ...
        .addOptional(EXPLOSION_RADIUS, radius)
        .build(BLOCK));
  }
  ...
  public static Supplier<LootContext> block(
      World world, Vec3d origin, BlockState state, @Nullable ItemStack stack) {
    return () -> build(setBuilder((ServerWorld) world)
        .add(ORIGIN, origin)
        .add(BLOCK_STATE, state)
        .add(TOOL, Objects.requireNonNullElse(stack, ItemStack.EMPTY))
