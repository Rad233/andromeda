...
public final class AndromedaClient {
  ...
  public void onInitializeClient(ModuleManager manager) {
    ...
    Andromeda.GROUP.ifPresent(g -> ItemGroupAnimaton.setIconAnimation(
        g, (group, context, itemX, itemY, selected, isTopRow) -> {
          try {
            if (!animate) return;
            drawTexture(
                context.getMatrices(), itemX + 8, itemY + 8, stack -> {}, BACKGROUND_TEXTURE);
            drawTexture(
                context.getMatrices(),
                itemX + 8,
                itemY + 8,
                stack -> stack.multiply(
                    RotationAxis.NEGATIVE_Z.rotationDegrees(Util.getMeasuringTimeMs() * 0.05f)),
                GALAXY_TEXTURE);
          } catch (Throwable t) {
            animate = false;
          }
  ...
  public static void drawTexture(
      MatrixStack matrices, int x, int y, Consumer<MatrixStack> transform, Identifier id) {
